...
import { getCompany, getCategories, getParties, getSources, getProfitAndLoss, getBalanceSheet, getCashFlow, listEntries, queryEntries, getTransaction, } from './api.js';
...
function resolveRefreshToken(req) {
    return req.headers['x-digits-refresh-token']
...
function createMcpServer(accessToken) {
    ...
    // ── Company ────────────────────────────────────────────────────────────────
    server.tool('get_company', 'Get company info: name, fiscal year start month, earliest transaction date. Call this first.', {}, async () => ok(await getCompany(accessToken)));
    // ── Chart of Accounts ──────────────────────────────────────────────────────
    server.tool('get_categories', 'Get the full Chart of Accounts from the ledger. Returns category IDs, names, parent hierarchy, and types (Assets, Liabilities, Equity, Income, Expenses, COGS, etc.). Use category IDs to filter transactions.', {}, async () => ok(await getCategories(accessToken)));
    // ── Vendors / Customers ────────────────────────────────────────────────────
    server.tool('get_parties', 'Get all parties (vendors, customers, contractors) from the ledger. Returns party IDs and names — e.g. "Google Cloud Platform", "Amazon Web Services", "Stripe". Use party IDs to filter transactions to specific vendors.', {}, async () => ok(await getParties(accessToken)));
    // ── Bank Feeds / Sources ──────────────────────────────────────────────────
    server.tool('get_sources', 'Get all connection sources (bank accounts, credit cards) linked to Digits.', {}, async () => ok(await getSources(accessToken)));
    // ── Financial Statements ───────────────────────────────────────────────────
    server.tool('get_profit_and_loss', 'Generate a Profit & Loss (Income Statement) for any date range. Returns hierarchical rows: Income → Gross Profit → Operating Expenses → Net Operating Income → Net Income. Supports monthly/quarterly/annual breakdown.', { ...dateParams, fiscal_year_start_month: z.number().int().min(1).max(12).optional().describe("Fiscal year start month 1-12 (default: January)") }, async ({ start_date, end_date, interval, fiscal_year_start_month }) => ok(await getProfitAndLoss(accessToken, {
        startDate: start_date, endDate: end_date, interval,
        fiscalYearStartMonth: fiscal_year_start_month?.toString(),
    })));
    server.tool('get_balance_sheet', 'Generate a Balance Sheet for any date. Returns Assets, Liabilities, and Equity with hierarchical line items.', { ...dateParams, fiscal_year_start_month: z.number().int().min(1).max(12).optional() }, async ({ start_date, end_date, interval, fiscal_year_start_month }) => ok(await getBalanceSheet(accessToken, {
        startDate: start_date, endDate: end_date, interval,
        fiscalYearStartMonth: fiscal_year_start_month?.toString(),
    })));
    server.tool('get_cash_flow', 'Generate a Cash Flow Statement for any date range. Returns Operating, Investing, Financing activities and Net Cash increase/decrease.', { ...dateParams, fiscal_year_start_month: z.number().int().min(1).max(12).optional() }, async ({ start_date, end_date, interval, fiscal_year_start_month }) => ok(await getCashFlow(accessToken, {
        startDate: start_date, endDate: end_date, interval,
        fiscalYearStartMonth: fiscal_year_start_month?.toString(),
    })));
    // ── Transactions ───────────────────────────────────────────────────────────
    server.tool('list_transactions', 'List recent ledger entries (transactions). Returns entries with date, amount, category, party, and description. Paginate with cursor.', {
        limit: z.number().int().min(1).max(100).optional().describe("Number of entries (max 100, default 100)"),
        cursor: z.string().optional().describe("Pagination cursor from previous response"),
    }, async ({ limit, cursor }) => ok(await listEntries(accessToken, {
        limit: limit?.toString(),
        cursor,
    })));
    server.tool('query_transactions', `Search and filter ledger entries. The most powerful reporting tool — filter by vendor (party), category, amount range, date range, or keyword. 
...
- Unpaid bills: linkedObjectType="Bill"`, {
        ...
        cursor: z.string().optional().describe("Pagination cursor from previous response"),
    }, async ({ occurred_after, occurred_before, minimum_amount, maximum_amount, filter_term, search_field, search_term, party_ids, category_ids, category_types, department_ids, transaction_type, linked_object_type, limit, cursor }) => ok(await queryEntries(accessToken, {
    ...
    server.tool('get_transaction', 'Get full details of a single ledger transaction by its Digits transaction ID.', { transaction_id: z.string().describe("Digits transaction ID") }, async ({ transaction_id }) => ok(await getTransaction(accessToken, transaction_id)));
    return server;
}
// ─── Auth middleware ───────────────────────────────────────────────────────────
